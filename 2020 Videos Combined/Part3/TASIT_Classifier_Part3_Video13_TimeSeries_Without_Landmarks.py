...
df = pd.read_csv('Part3 Video13 Vectorized R01.csv',header=None)
...
participant = df.pop(15006).values
...
label = df.pop(15005).values
#taking only the columns related to features excluding facial landmarks
df1 = df.iloc[:,0:4000] #3000:4000, consistent with different random seed
df2 = df.iloc[:,5000:6000]
df3 = df.iloc[:,9000:11000]
df4 = df.iloc[:,12000:15000]
df = pd.concat([df1,df2,df3,df4], axis=1)

np.savetxt('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\part3video13_firstpaper_participant.csv',participant,delimiter=',')
np.savetxt('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\part3video13_firstpaper_label.csv',label,delimiter=',')
...
df2 = df
X_train2, X_test2, label_train2, label_test, participant_train, participant_test = train_test_split(df2, label, participant, stratify=label,test_size=0.20, random_state=500, shuffle=True)
...
np.savetxt('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\part3video13_firstpaper_participant_test.csv',participant_test,delimiter=',')
np.savetxt('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\part3video13_firstpaper_label_test.csv',label_test,delimiter=',')
...
for i in range(NUM_TRIALS):
    X_train, X_val, label_train, label_val = train_test_split(X_train2, label_train2, test_size=0.20,
                                     random_state=1099, stratify=label_train2)
    ...
    #                                n_samples=len(X_train[label_train == 1])-len(X_train[label_train == 0]),
    #                                random_state=134)
    #
    ...
    X_test = lda.transform(X_test2)
    df = lda.transform(df2)
    np.savetxt('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\part3video13_firstpaper_test.csv',X_test,delimiter=',')

    #doing principal component analysis on the train data
    # pca = KernelPCA(n_components=round(len(X_train)), kernel='poly', degree=1, fit_inverse_transform=True,random_state=200,
    #                         eigen_solver='dense', gamma = 1,remove_zero_eig=True)
    ...
    # X_train["label"] = label_train
    # X_train_min = resample(X_train[label_train == 0], replace=True,
    #                        n_samples=2*(len(X_train[label_train == 1])-len(X_train[label_train == 0])),
    #                        random_state=134)
    #
    # X_train = pd.concat([X_train, X_train_min])
    # label_train = X_train.pop("label").values
    ...
    class_weight = {0:3*count_correct/count_incorrect, 1: 1}
    class_weight1 = {0:count_correct/count_incorrect, 1: .8}
    ...
    rf = RandomForestClassifier(n_estimators=100, max_features="sqrt", criterion='gini',
                                max_depth=2, min_samples_split=2,min_impurity_decrease=0.11,
                                class_weight=class_weight1, random_state=random_state[i])
    # rf = TimeSeriesForest(min_window_size=1,max_depth=3,class_weight=class_weight1)
    # xgb = LinearDiscriminantAnalysis()
    # mnb = KNeighborsClassifier(p=3, n_neighbors = 12, weights='uniform')
    #mnb = clustering.KShape(n_clusters=2)
    # sv = TimeSeriesSVC(kernel='poly', degree= 1, coef0=1.0, C= 10, probability= True,
    #             class_weight=class_weight1)
    sv = TimeSeriesSVC(kernel = "gak", gamma='auto',probability = True,
                 class_weight=class_weight1, random_state=random_state[i])
    # sv = svm.SVC(kernel='poly', degree=1, probability= True,
    #              class_weight=class_weight1)
...
pickle.dump(vc, open('C:/Users\siffa\.PyCharmCE2018.2\config\scratches\model_part3_video13_salient_without', 'wb'))
